...
modDat <- readRDS( here("Data_processed", "CoverData", paste0("DataForModels_spatiallyAveraged_withSoils_noSf_sampledLANDFIRE", suffix, 
...
climDat_Temp <- readRDS(paste0("../../../Data_processed/WallToWallClimateData/DayMetData_allCONUS_2023ClimateValues_withSoils"
                               , suffix, ".rds"))
...
if (run == TRUE) {
  ...
  my_folds <- as.numeric(as.factor(train_eco))
    ...
    base::saveRDS(fit, paste0("../ModelFitting/models/yesOrNoTrees_globalLASSOmod_binomial_treeCutoff_",treeThreshold, suffix, ".rds"))
...
## fit
  ...
  ## save models 
  saveRDS(fit_glm_bestLambda, paste0("./models/yesOrNoTrees_bestLambdaGLM_",treeThreshold, suffix, ".rds"))
  saveRDS(fit_glm_halfse, paste0("./models/yesOrNoTrees_halfSELambdaGLM_",treeThreshold, suffix, ".rds"))
  saveRDS(fit_glm_se, paste0("./models/yesOrNoTrees_oneSELambdaGLM_",treeThreshold, suffix, ".rds"))
  ...
  save(f_glm_halfse, mat_glmnet_halfse, halfseLambda_coef,
              ...
              file = paste0("./models/interimModelFittingObjects_yesOrNoTrees_binomial_",treeThreshold, suffix, ".rds"))
  } else {
    # read in LASSO object
    fit <- readRDS(paste0("../ModelFitting/models/yesOrNoTrees_globalLASSOmod_binomial_treeCutoff_",treeThreshold, suffix, ".rds"))
    
    # read in R objects having to do w/ model fitting 
    load(file = paste0("./models/interimModelFittingObjects_yesOrNoTrees_binomial_",treeThreshold, suffix, ".rds"))
      
  fit_glm_bestLambda <- readRDS(paste0("./models/yesOrNoTrees_bestLambdaGLM_",treeThreshold, suffix, ".rds"))
  fit_glm_halfse <- readRDS(paste0("./models/yesOrNoTrees_halfSELambdaGLM_",treeThreshold, suffix, ".rds"))
  fit_glm_se <- readRDS(paste0("./models/yesOrNoTrees_oneSELambdaGLM_",treeThreshold, suffix, ".rds"))
  }
...
climDatPred <- readRDS(paste0("../../../Data_processed/WallToWallClimateData/DayMetData_allCONUS_2023_SCALEDcliamteAndSoilsPredictors", suffix, ".rds"))
```
...
saveRDS(best_threshold_bestLambda, paste0("./models/YesNoTreeModelPrevalence", suffix, ".rds"))
...
## save this prevalence for later 
saveRDS(best_threshold_secondBest, paste0("./models/YesNoTreeModelPrevalence_secondBest", suffix, ".rds"))
...
# this file created in the 05_ rmd so this section can only run after that's been run. 
p_fp <- paste0("../../../Data_processed/CoverData/finalCoverData/predictionsOfForestProbOnDaymetScale", suffix, ".tif") # forest preds = fp

if(file.exists(p_fp)) {
  exists_fp <- TRUE
  forestPreds_dmGrid <- terra::rast(p_fp)
} else {
  exists_fp <- FALSE
}
...
```{r classification error map for best lambda model Only in Unforested Locations, fig.height = 6.5, fig.width = 9, warning = FALSE, eval = exists_fp }
...
```{r classification error map for second-best lambda model Only in Unforested Locations, fig.height = 6.5, fig.width = 9, warning = FALSE, eval = exists_fp }
## mask data based on forest classification
